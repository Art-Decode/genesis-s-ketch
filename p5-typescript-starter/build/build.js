...
function Scribble(p) {
    ...
    this.buildEllipse = function (cx, cy, rx, ry, offset, overlap) {
        ...
        this.sketch.curveVertex(this.getOffset(-offset, offset) +
            cx +
            0.9 * rx * Math.cos(radialOffset - this.ellipseInc), this.getOffset(-offset, offset) +
            cy +
        ...
        for (var theta = radialOffset; theta < Math.PI * 2 + radialOffset - 0.01; theta += this.ellipseInc) {
            this.sketch.curveVertex(this.getOffset(-offset, offset) + cx + rx * Math.cos(theta), this.getOffset(-offset, offset) + cy + ry * Math.sin(theta));
        }
        this.sketch.curveVertex(this.getOffset(-offset, offset) +
            cx +
            rx * Math.cos(radialOffset + Math.PI * 2 + overlap * 0.5), this.getOffset(-offset, offset) +
            cy +
            ry * Math.sin(radialOffset + Math.PI * 2 + overlap * 0.5));
        this.sketch.curveVertex(this.getOffset(-offset, offset) +
            cx +
            0.98 * rx * Math.cos(radialOffset + overlap), this.getOffset(-offset, offset) +
            cy +
            0.98 * ry * Math.sin(radialOffset + overlap));
        this.sketch.curveVertex(this.getOffset(-offset, offset) +
            cx +
            0.9 * rx * Math.cos(radialOffset + overlap * 0.5), this.getOffset(-offset, offset) +
            cy +
    ...
    this.scribbleLine = function (x1, y1, x2, y2) {
        ...
        this.sketch.curveVertex(midDispX +
            x1 +
            (x2 - x1) * divergePoint +
            this.getOffset(-offset, offset), midDispY + y1 + (y2 - y1) * divergePoint + this.getOffset(-offset, offset));
        this.sketch.curveVertex(midDispX +
            x1 +
            2 * (x2 - x1) * divergePoint +
            this.getOffset(-offset, offset), midDispY +
            y1 +
        ...
        this.sketch.curveVertex(midDispX +
            x1 +
            (x2 - x1) * divergePoint +
            this.getOffset(-halfOffset, halfOffset), midDispY +
            y1 +
            (y2 - y1) * divergePoint +
    ...
    this.scribbleCurve = function (x1, y1, x2, y2, x3, y3, x4, y4) {
        this.sketch.bezier(x1 + this.getOffset(-2, 2), y1 + this.getOffset(-2, 2), x3 + this.getOffset(-4, 4), y3 + this.getOffset(-3, 3), x4 + this.getOffset(-3, 3), y4 + this.getOffset(-3, 3), x2 + this.getOffset(-1, 1), y2 + this.getOffset(-1, 1));
        this.sketch.bezier(x1 + this.getOffset(-2, 2), y1 + this.getOffset(-2, 2), x3 + this.getOffset(-3, 3), y3 + this.getOffset(-3, 3), x4 + this.getOffset(-3, 3), y4 + this.getOffset(-4, 4), x2 + this.getOffset(-2, 2), y2 + this.getOffset(-2, 2));
    };
...
var Particle = (function () {
    function Particle(x, y, r) {
        ...
        if (getRndInteger(0, 100) > 50) {
            stroke(getRndInteger(0, 255), getRndInteger(0, 255), getRndInteger(0, 255));
...
function draw() {
    ...
    while (area > MAX_PARTICLES_RADIUS) {
        ...
        var r = random(1, MAX_PARTICLES_RADIUS);
        var collision = false;
        for (var i_1 in particles) {
            var d = dist(x, y, particles[i_1].x, particles[i_1].y);
            if (d - r - particles[i_1].r <= 0)
                collision = true;
        }
        if (!collision) {
            particles.push(new Particle(x, y, r));
            area -= Math.pow(r, 2) * PI * 1.5;
        }
    }
}
